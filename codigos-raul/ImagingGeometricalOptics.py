...
Modified on Friday July 7th 17:01:54 2023
...
def ray_tracing(width, height, rayo, so, n1, obj, res, nl, R1, R2, dl, aberration, pixels):
    ...
    for i in range(width):
        for j in range(height):
            ...
            #SPherical aberration according to Siedel terms
            if aberration and rayo:
              C = 1000  #This is the weight of the aberration (C=0 means no aberration). Recommended: 1000
            else:
              C = 0

            #Focal distance for each point
            f = ((nl - 1) * ((1/R1) - (1/R2)))
            f = f +  (C*(y_objeto**4 - y_objeto**2)) #This is only useful when modeling aberration
            f = 1/f
...
if __name__ == "__main__":
    ...
    res = 0.0001 #each pixel equals res
